...
def open_camera_window():
    ...
    # Capture button (centered in row 0, column 0)
    ...
    bsend = Button(text="Start", height=2, width=10, command= send )
...
def manual_input_window():
    ...
    # Output text boxes
    display_solution = Text(height=4, width=55)
    ...
    bsolve = Button(text="Solve", height=2, width=10, command=solve)
    bsend = Button(text="Start", height=2, width=10, command=send )
    canvas.create_window(10 + 10.5 * width, 10 + 6.5 * width, anchor=NW, window=bsolve)
    canvas.create_window(10 + 10.5 * width, 10 + 7.5 * width, anchor=NW, window=Button(text="Clean", width=10, command=clean))
    canvas.create_window(10 + 10.5 * width, 10 + 8 * width, anchor=NW, window=Button(text="Empty", width=10, command=empty))
    canvas.create_window(10 + 10.5 * width, 10 + 8.5 * width, anchor=NW, window=Button(text="Random", width=10, command=random))
...
Label(pattern_select_frame, text="     Ms.Cube", bg="white", font=("Arial", 25, "bold")).pack(pady=20)
Label(pattern_select_frame, image=photo, bg="white").pack(pady=5)
